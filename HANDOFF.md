...
## Completed Work
*   **Database Setup**: Complete schema defined for `User`, `Service` (with `buffer_minutes`), `Availability`, and `Appointment`, including NextAuth adapter models.
...
## System State
All files are cleanly structured using the App Router (`src/app`). Standard global `layout.tsx` is defined. The MVP is fully implemented and ready for deployment or the next iteration of feature enhancements.
