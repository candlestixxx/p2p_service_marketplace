...
* [ ] Refactor `isRedirectError` import: Change the internal import path in `src/actions/auth.ts` to `import { isRedirectError } from "next/navigation"` once framework supports it stably without build error.
